#!/usr/bin/env bun
/**
 * Discord Weekly Reports - Cron Script
 * 
 * This script generates weekly reports for all Discord channels.
 * It should be scheduled to run every Monday via system cron (not in-app).
 * 
 * Schedule with crontab:
 *   0 9 * * 1 cd /path/to/bio-internal && bun run scripts/discord-weekly-reports-cron.ts
 * 
...
async function main() {
  console.log('📊 Starting Weekly Discord Report Generation');
  ...
  try {
    ...
    for (let i = 0; i < channels.length; i++) {
      ...
      try {
        console.log(`${progress} 📝 Generating weekly report for: ${channel.name} (${channel.channelId})`);
    ...
    console.error('\n❌ Fatal error during weekly report generation:', error);
    process.exit(1);
  }
}
