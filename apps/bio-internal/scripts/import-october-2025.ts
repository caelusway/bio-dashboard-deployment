#!/usr/bin/env bun
/**
 * Import October 2025 Marketing Data - ALL PLATFORMS
 *
 * Imports complete October 2025 data from CSV
 */
...
async function importOctoberData(csvPath: string) {
  ...
  // Parse Website metrics - October is on rows 16-17 (lines are 1-indexed, array is 0-indexed, so row 16 = rows[15])
  console.log('\n📈 Parsing website metrics for October...');

  // Row 16 (array index 15): October, bio.xyz (30359 views, 14543 active, 13000 new)
  const bioRow = rows[15];
  if (bioRow && bioRow[1] === 'October') {
    const pageViews = cleanNumber(bioRow[3]);
    const activeUsers = cleanNumber(bioRow[4]);
    const newUsers = cleanNumber(bioRow[5]);
  ...
  // Row 17 (array index 16): October, app.bio.xyz (782400 views, 36754 active, 23533 new)
  const appRow = rows[16];
  if (appRow && appRow[1] === 'October') {
    const pageViews = cleanNumber(appRow[3]);
    const activeUsers = cleanNumber(appRow[4]);
    const newUsers = cleanNumber(appRow[5]);
  ...
  // Parse Social Follower Count - October is on row 29 (array index 28)
  console.log('\n📱 Parsing social follower metrics for October...');
  const socialRow = rows[28];
  if (socialRow && socialRow[1] === 'October') {
    ...
    console.log(`  X (Twitter): ${xFollowers} followers`);
    console.log(`  LinkedIn: ${linkedinFollowers} followers`);
    console.log(`  YouTube: ${youtubeFollowers} subscribers`);
    console.log(`  Telegram: ${telegramMembers} members`);
    console.log(`  Discord: ${discordMembers} members`);
    console.log(`  Bluesky: ${blueskyFollowers} followers`);
  ...
  // Parse X/Twitter engagement metrics - October is on row 44 (array index 43)
  console.log('\n🐦 Parsing X (Twitter) engagement metrics for October...');
  const xMetricsRow = rows[43];
  if (xMetricsRow && xMetricsRow[1] === 'October') {
    ...
    const replies = cleanNumber(xMetricsRow[7]); // 7500
    const bookmarks = cleanNumber(xMetricsRow[8]); // 950
    const reposts = cleanNumber(xMetricsRow[9]); // 5600

    console.log(`  Engagements: ${engagements}`);
    console.log(`  Impressions: ${impressions}`);
    console.log(`  Profile visits: ${profileVisits}`);
    console.log(`  Replies: ${replies}, Bookmarks: ${bookmarks}, Reposts: ${reposts}`);
  ...
  // Parse Discord metrics - October is on row 58 (array index 57)
  console.log('\n💬 Parsing Discord metrics for October...');
  const discordRow = rows[57];
  if (discordRow && discordRow[1] === 'October') {
    const members = cleanNumber(discordRow[3]); // 8562
    console.log(`  Members: ${members}`);
    // Already added in social follower section
  }

  // Parse YouTube views - October is on row 84 (array index 83)
  console.log('\n🎥 Parsing YouTube view metrics for October...');
  const youtubeRow = rows[83];
  ...
  // Parse Luma metrics - October is on row 103 (array index 102)
  console.log('\n📅 Parsing Luma metrics for October...');
  const lumaRow = rows[102];
  ...
  console.log('\n📋 Summary by platform:');
  console.log('   - Website (bio.xyz): page views, active users, new users');
  console.log('   - Website (app.bio.xyz): page views, active users, new users');
  console.log('   - Twitter/X: followers, engagements, impressions');
  console.log('   - LinkedIn: followers');
  console.log('   - YouTube: subscribers, views, impressions');
  console.log('   - Telegram: members');
  console.log('   - Discord: members');
  console.log('   - Luma: page views, subscribers');
}

async function main() {
  console.log('🚀 Starting October 2025 Data Import (ALL PLATFORMS)...\n');
